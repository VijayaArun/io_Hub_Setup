...
kubectl --namespace kube-system create serviceaccount geo
kubectl create clusterrolebinding geo --clusterrole cluster-admin --serviceaccount=kube-system:geo
helm init --service-account geo
kubectl patch deployment geo-deploy --namespace=kube-system --type=json --patch='[{"op": "add", "path": "/spec/template/spec/containers/0/command", "value": ["/geo", "--listen=localhost:44134"]}]'
